...
export function initPricing() {
      ...
      <div class="pricing-grid">
        ${PLANS.map((p) => `
          <div class="pricing-card ${p.best ? 'best' : ''}">
            ...
            <div class="pricing-per">${p.buyout ? `一次买断 · 相当于 ¥${buyoutPerMonth}/月` : '约 ¥' + p.perMonth + ' / 月'}</div>
            <ul class="pricing-feats">
              <li><i class="ti ti-check"></i>解锁全部校招岗位 · 无限翻页</li>
              <li><i class="ti ti-check"></i>投递入口查看不限次数</li>
              <li><i class="ti ti-check"></i>简历 / AI 匹配 / 复盘全解锁</li>
            </ul>
  ...
  function showQrModal(order) {
    ...
    // 支付成功：刷新会员状态 → 提示 → 关闭弹窗 → 跳首页
    async function showSuccessAndClose(ov, timer) {
      clearInterval(timer);
      // 关键：支付成功必须同步本地会员标志，否则 isMember() 仍为 false，用户会被 5 页限制卡住
      try { await Auth.syncMemberStatus(); } catch { /* 刷新失败也不阻塞跳转 */ }
